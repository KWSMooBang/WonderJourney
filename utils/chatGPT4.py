...
class TextpromptGen(object):
    ...
    def regenerate_background(self, style, entities, scene_name, background=None):
        if background is not None:
            content = "Please generate a brief scene background with Scene name: " + scene_name \
                + "; Background: " + str(background).strip(".") + ". Entities: " + str(entities) + "; Style: " + str(style)
        else:
            content = "Please generate a brief scene background with Scene name: " + scene_name \
        ...
        messages = [
            {
                'role': 'system',
                'content': "You are an intelligent scene generator. Given a scene and there are 3 most significant common entities. please generate a brief background prompt about 50 words describing common things in the scene. You should not mention the entities in the background prompt. If needed, you can make reasonable guesses."
            },
            {
                'role': 'user',
                'content': content
    ...
    def run_conversation(self, style=None, entities=None, scene_name=None, background=None, control_text=None):
        ...
        if control_text is not None:
            ...
            if background is not None:
                ...
                scene_content = "\nScene " + str(self.scene_num) + ": " + "{Background: " + str(background).strip(".")  \
                    + ". Entities: " + str(entities) + "; Style: " + str(style) + "}"
            else: 
        ...
        if control_text is not None:
            messages = [
                {
                    'role': 'system',
                    'content': "You are an intelligent scene description generator. Given a sentence describing a scene, please translate it into English if not and summarize the scene name and 3 most significant common entities in the scene. You also have to generate a brief background prompt about 50 words describing the scene. You should not mention the entities in the background prompt. If needed, you can make reasonable guesses. Please use the format below: (the output should be json format)\n \
        ...
        for i in range(10):
            try:
                ...
                try:
                    ...
                    user_message = {
                        'role': 'user',
                        'content': "The output is not json format, please try again:\n" + self.content
                    }
                    messages.append(assistant_message)
                    messages.append(user_message)
                    print("An error occured when transfering the output of chatGPT into a dict, chatGPT4, let's try again!", str(e))
                    continue
            except openai.APIError as e:
                print(f"OpenAI API returned an API Error: {e}")
                print("Wat for a second and ask chatGPT4 again!")
                time.sleep(1)
                continue
    ...
    def encoder_image_pil(self, image):
        with io.BytesIO() as buffer:
            image.save(buffer, format='PNG')
            return base64.b64encode(buffer.getvalue()).decoder('utf-8')
        
    def evaluate_image(self, image, eval_blur=True):
        api_key = openai.api_key
        base64_image = self.encoder_image_pil(image)
        ...
        if eval_blur:
            ...
            for i in range(5):
                try:
                    ...
                    print("Something has been wrong while asking GPT4. Wait for a second and ask chatGPT4 again!")
                    time.sleep(1)
                    continue
            has_blur = blur == 'yes'
        else:
